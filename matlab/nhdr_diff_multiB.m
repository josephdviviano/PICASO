...
% %   end
  ...
  %b = sqrt(sum(dwi.gradientdirections.^2,2)); %this is for phantom data, and MGH
  b = (sum(dwi.gradientdirections.^2,2)); %for in-vivo data
  
  dwi.gradientdirections(b>=0.05,:) = dwi.gradientdirections(b>=0.05,:)./sqrt(b(b>=0.05,ones(1,3)));
